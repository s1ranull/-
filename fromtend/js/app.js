...
function applyProfileTheme(profile) {
  document.body.classList.remove("theme-default", "theme-dark", "theme-gold", "frame-classic", "frame-premium");
  const theme = profile?.theme || "default";
  const frame = profile?.avatar_frame || "none";
  document.body.classList.add(`theme-${theme}`);
  if (frame !== "none") document.body.classList.add(`frame-${frame}`);

  const topbarBadges = document.getElementById("topbarBadges");
  if (topbarBadges) {
    const badges = profile?.badges || [];
    topbarBadges.innerHTML = badges.map(b => `<span class="pill">${esc(b)}</span>`).join("");
  }
}
...
function setTopbar() {
  ...
  profileLink.style.display = u ? "inline-block" : "none";
}

logoutBtn.onclick = () => {
  clearSession();
  selectedCourseId = null;
  applyProfileTheme({ theme: "default", avatar_frame: "none", badges: [] });
  setTopbar();
  location.hash = "#/login";
};
...
async function requireAuth() {
  ...
  try {
    ...
    setTopbar();
    try { applyProfileTheme(await api.getProfile()); } catch {}
    return u;
  } catch {
    clearSession();
    setTopbar();
...
async function renderUserWidgets(right, onSaved = null) {
  ...
  wrap.innerHTML = `
    <div class="hr"></div>
    <div class="h1" style="font-size:22px;">Профиль</div>
    <div class="item">
      <div><b>${esc(profile.username)}</b></div>
      <div class="muted">Тема: ${esc(profile.theme)}</div>
      <div class="muted">Рамка: ${esc(profile.avatar_frame)}</div>
      <div class="muted">Значки: ${(profile.badges || []).map(esc).join(", ") || "нет"}</div>
      <div class="pills">${(profile.badges || []).map(b => `<span class="pill">${esc(b)}</span>`).join("")}</div>
      ...
      <div class="field">
        <label>Значки (через запятую)</label>
        <input id="profileBadges" placeholder="top-student, fast-learner" />
      </div>

      <button class="btn ghost" id="saveProfileBtn">Применить профиль</button>
    </div>

    <div class="hr"></div>
    <div class="h1" style="font-size:22px;">Уведомления</div>
    <div class="list" id="notificationsBox">
      ${
        notifications.length
          ? notifications.map(n => `
              <div class="item">
                <b>${esc(n.event_type)}</b><br/>
                ${esc(n.message)}
              </div>
            `).join("")
          : `<div class="item muted">Пока уведомлений нет</div>`
      }
    </div>
  `;
  ...
  wrap.querySelector("#profileTheme").value = profile.theme || "default";
  wrap.querySelector("#profileFrame").value = profile.avatar_frame || "none";
  wrap.querySelector("#profileBadges").value = (profile.badges || []).join(", ");

  wrap.querySelector("#saveProfileBtn").onclick = async () => {
    try {
      const theme = wrap.querySelector("#profileTheme").value;
      const avatar_frame = wrap.querySelector("#profileFrame").value;
      const badges = wrap.querySelector("#profileBadges").value
        .split(",")
        .map(x => x.trim())
        .filter(Boolean);

      const updated = await api.customizeProfile({ theme, avatar_frame, badges });
...
async function viewProfile() {
  ...
  const node = h(`
    <div class="grid">
      <section class="card">
        <div class="h1">Кастомизация профиля</div>
        <div class="muted">Тут отдельно вынесены настройки профиля и уведомления.</div>
      </section>
      <section class="card" id="profileRight"></section>
    </div>
  `);
  app.appendChild(node);
  await renderUserWidgets(node.querySelector("#profileRight"), async () => {
    await viewProfile();
  });
}
...
async function viewCourses() {
  ...
  const node = h(`
    <div class="grid">
      ...
      <section class="card" id="right">
        <div class="h1">Панель студента</div>
        <div class="muted">Выбери курс. Для кастомизации профиля есть отдельная кнопка «Профиль» сверху.</div>
      </section>
    </div>
  `);
  ...
  try {
    ...
    if (!Array.isArray(courses) || courses.length === 0) {
      list.innerHTML = `<div class="item muted">Курсы пока не найдены.</div>`;
      return;
    }

    courses.forEach((c) => {
      const locked = !c.has_access;
      const item = h(`
          ...
          <div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start;">
            <div>
              ...
              <div class="pills">
                ...
                <span class="pill">${c.is_free ? "бесплатно" : `${Number(c.price || 0).toFixed(2)} credits`}</span>
                <span class="pill">${esc(c.state_label || (locked ? "Закрыт" : "Доступен"))}</span>
              </div>
            </div>
            <button class="btn ${locked ? "primary" : "ghost"}">${esc(c.action_label || (locked ? "Открыть и оплатить" : "Открыть"))}</button>
          </div>
        </div>
      `);
      item.querySelector("button").onclick = async () => {
        ...
        try {
          await openCourse(c.id, right);
        } catch (e) {
          right.innerHTML = `<div class="h1">Ошибка</div><div class="item bad">${esc(e.message || e)}</div>`;
          showToast(`Ошибка открытия курса: ${e.message || e}`);
        }
      };
    ...
    if (selectedCourseId) {
      try {
        await openCourse(selectedCourseId, right);
      } catch {
        selectedCourseId = null;
      }
    }
  } catch (e) {
    list.innerHTML = `<div class="item bad">Не удалось загрузить курсы: ${esc(e.message || e)}</div>`;
    right.innerHTML = `<div class="h1">Ошибка</div><div class="item bad">Проверь backend и авторизацию.</div>`;
    showToast(`Курсы: ${e.message || e}`);
  }
}
    ...
    try {
      ...
      res.innerHTML = `
        <div class="item ${ok ? "good" : "bad"}">
          <b>Результат:</b> ${r.percent}% (${r.points}/${r.max_points})<br/>
          <span class="muted">Стратегия до: ${esc(r.difficulty_strategy_before || "-")} → после: ${esc(r.difficulty_strategy_after || "-")}</span><br/>
          <span class="muted">Состояние курса: ${esc(r.course_state_after?.state_label || "-")}</span>
          <div style="margin-top:10px;">
            <button type="button" class="btn ghost" id="retryTestBtn">Пройти тест снова</button>
          </div>
        </div>
      `;
      const retryBtn = res.querySelector("#retryTestBtn");
      if (retryBtn) retryBtn.onclick = async () => openCourse(courseId, mount);
...
async function openCourse(courseId, mount) {
  if (!mount) throw new Error("Не найден блок для отображения курса");
  selectedCourseId = courseId;
  mount.innerHTML = `<div class="h1">Загрузка курса</div><div class="item muted">Подожди, загружаю данные…</div>`;

  let c;
  let access;
  try {
    c = await api.course(courseId);
    access = await api.courseAccess(courseId);
  } catch (e) {
    mount.innerHTML = `<div class="h1">Ошибка</div><div class="item bad">Не удалось загрузить курс: ${esc(e.message || e)}</div>`;
    throw e;
  }

  const locked = !access.has_access;
  const stateHtml = `
    <div class="hr"></div>
    <div class="h1" style="font-size:22px;">Состояние курса</div>
    <div class="item">
      <b>${esc(access.state_label || (locked ? "Закрыт" : "Доступен"))}</b><br/>
      <span class="muted">${esc(access.state_message || "")}</span>
    </div>
  `;

  const paymentHtml = `
    <div class="hr"></div>
    <div class="h1" style="font-size:22px;">Оплата курса</div>
    <div class="item">
      <div class="muted">Цена курса задаётся администратором.</div>
      <div style="margin:8px 0 12px 0;">Стоимость: <b>${Number(access.price || 0).toFixed(2)}</b></div>
      <div class="field">
      ...
      <button class="btn primary" id="payBtn">${locked ? "Оплатить и открыть" : "Курс уже оплачен"}</button>
      <div id="paymentBox" style="margin-top:10px;"></div>
    </div>
  `;
  ...
  if (locked) {
    mount.innerHTML = `
      ...
      <div class="muted">${esc(c.description)}</div>
      <div class="item bad" style="margin-top:12px;">${esc(access.state_message || "Этот курс закрыт до оплаты. После оплаты откроются материал, тест, аналитика и сертификат.")}</div>
      ${stateHtml}
      ${paymentHtml}
    `;

    mount.querySelector("#payBtn").onclick = async () => {
      const provider = mount.querySelector("#paymentProvider").value;
      const box = mount.querySelector("#paymentBox");
      try {
        const res = await api.payCourse({ course_id: courseId, provider });
        box.innerHTML = `<div class="item good"><b>${esc(res.provider)}</b><br/>${esc(res.message)}<br/><span class="muted">TX: ${esc(res.transaction_id || "-")}</span></div>`;
        showToast("Курс оплачен и разблокирован");
        await viewCourses();
      } catch (e) {
        box.innerHTML = "";
        showToast(`Ошибка оплаты: ${e.message}`);
      }
    };
    return;
  }
  ...
  const difficulty = hasTest ? await api.difficulty(courseId) : null;
  ...
  const strategyHtml = hasTest ? `
    <div class="hr"></div>
    <div class="h1" style="font-size:22px;">Адаптивная сложность</div>
    <div class="item">
      Текущая стратегия: <b>${esc(difficulty.strategy)}</b><br/>
      <span class="muted">${esc(difficulty.message)}</span><br/>
      <span class="muted">Количество вопросов сейчас: <b>${difficulty.question_count}</b></span>
    </div>
  ` : "";

  const lmsHtml = `
    <div class="hr"></div>
    <div class="h1" style="font-size:22px;">Синхронизация с LMS</div>
    <div class="item">
      <div class="field">
        <label>LMS</label>
        <select id="lmsProvider">
          <option value="moodle">Moodle</option>
          <option value="googleclassroom">Google Classroom</option>
        </select>
      </div>
      <button class="btn ghost" id="syncBtn">Синхронизировать</button>
  ...
  mount.innerHTML = `
    ...
    <div class="muted">${esc(c.description)}</div>
    <div class="pills">
      <span class="pill">${c.is_free ? "бесплатно" : `оплачено: ${Number(access.price || 0).toFixed(2)}`}</span>
      <span class="pill">доступ открыт</span>
    </div>
  ...
  mount.querySelector("#syncBtn").onclick = async () => {
    const provider = mount.querySelector("#lmsProvider").value;
    const box = mount.querySelector("#syncBox");

    try {
      const res = await api.syncLms({ course_id: courseId, provider });
      box.innerHTML = `<div class="item good"><b>${esc(res.provider)}</b><br/>${esc(res.message)}<br/><span class="muted">REF: ${esc(res.external_ref || "-")}</span></div>`;
      showToast("Синхронизация выполнена");
    } catch (e) {
      box.innerHTML = "";
      showToast(`Ошибка LMS: ${e.message}`);
    }
  };
...
async function viewAdmin() {
  ...
  const node = h(`
    <div class="grid">
      <section class="card">
        ...
        <div class="field"><label>Цена курса</label><input id="coursePrice" type="number" min="0" step="0.01" value="0" /></div>
  ...
  const coursePrice = node.querySelector("#coursePrice");
...
async function router() {
  ...
  if (route.startsWith("/profile")) return viewProfile();
